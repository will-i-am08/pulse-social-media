...
export default function CreatePostPage() {
  ...
  const [qualityChecked, setQualityChecked] = useState([false, false, false])
  const [bulkQualityChecked, setBulkQualityChecked] = useState([false, false, false])
  const allQualityChecked = qualityChecked.every(Boolean)
  const allBulkQualityChecked = bulkQualityChecked.every(Boolean)
  ...
  function savePost(status: Post['status']) {
    ...
    const newPost: Post = {
      ...
      quality_checked: allQualityChecked,
    }
  ...
  async function fetchFromBlogUrl() {
    ...
    try {
      ...
      if (data.caption) { setCaption(cleanCaption(data.caption)); setQualityChecked([false, false, false]) }
      setCategory('blog')
  ...
  async function bulkGenerateAll() {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const content = row.images.length
        ...
        : engineOutput.userPrompt
      const result = await callClaude(engineOutput.systemPrompt, content, 768)
      rows[i] = { ...rows[i], caption: result ? cleanCaption(result) : '', status: result ? 'done' : 'idle' }
      setBulkRows([...rows])
    }
    setBulkGenerating(false)
    setBulkQualityChecked([false, false, false])
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <button className="btn btn-o flex items-center gap-2" disabled={!bulkRows.some(r => r.caption) || !allBulkQualityChecked} onClick={bulkSaveAllDrafts}>
            <BookmarkIcon className="w-4 h-4" /> Save All as Drafts
          </button>
        </div>

        {/* Bulk Quality Review Checklist */}
        {bulkRows.some(r => r.caption) && (
          <div className="border border-amber-500/40 bg-amber-500/5 rounded-lg p-4 space-y-2.5 mt-4">
            <p className="text-xs font-semibold text-amber-400 uppercase tracking-wide">Quality Check — All Posts</p>
            {[
              'Do these captions accurately describe what\'s in the images/videos?',
              'Have you rewritten at least 50% of each caption in the brand\'s voice?',
              'Are these free of AI-sounding language?',
            ].map((item, i) => (
              <label key={i} className="flex items-start gap-2.5 cursor-pointer">
                <input
                  type="checkbox"
                  className="w-4 h-4 mt-0.5 accent-amber-500"
                  checked={bulkQualityChecked[i]}
                  onChange={e => {
                    const next = [...bulkQualityChecked]
                    next[i] = e.target.checked
                    setBulkQualityChecked(next)
                  }}
                />
                <span className="text-sm text-[#e1bec0]">{item}</span>
              </label>
            ))}
            {!allBulkQualityChecked && (
              <p className="text-[10px] text-amber-500/70 italic">Complete all checks to enable saving</p>
            )}
          </div>
        )}
          ...
          return bb?.buffer_profile_ids?.length ? (
            <div className="card p-5 mt-5">
              ...
              <button
                ...
                disabled={bulkScheduling || !allBulkQualityChecked}
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          {caption && (
            <div className="border border-amber-500/40 bg-amber-500/5 rounded-lg p-4 space-y-2.5">
              <p className="text-xs font-semibold text-amber-400 uppercase tracking-wide">Quality Check</p>
              {[
                'Does this caption accurately describe what\'s in the image/video?',
                'Have you rewritten at least 50% of this caption in the brand\'s voice?',
                'Is this free of AI-sounding language?',
              ].map((item, i) => (
                <label key={i} className="flex items-start gap-2.5 cursor-pointer">
                  <input
                    type="checkbox"
                    className="w-4 h-4 mt-0.5 accent-amber-500"
                    checked={qualityChecked[i]}
                    onChange={e => {
                      const next = [...qualityChecked]
                      next[i] = e.target.checked
