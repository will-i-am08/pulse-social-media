...
export default function OverviewPage() {
  const { brands, posts: allPosts, loading } = useWorkspace()
  const { activeBrandId, activeBrand } = useActiveBrand()
  const posts = filterByActiveBrand(allPosts, activeBrandId)
  ...
  const stats = [
    ...
    { label: 'Drafts', val: drafts.length, Icon: DocumentTextIcon, href: '/drafts' },
    { label: 'Scheduled', val: scheduled.length, Icon: CalendarIcon, href: '/queue' },
    { label: 'Published', val: published.length, Icon: CheckCircleIcon, href: '/sent' },
  ]
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
        <div>
          <h1 className="text-3xl font-bold text-[#e6e1e1]">Overview</h1>
          <p className="text-[#e1bec0] mt-1 text-sm">
            {activeBrand ? `${activeBrand.name} at a glance` : 'All brands at a glance'}
          </p>
        </div>
      ...
      {submitted.length > 0 && (
        <div className="mb-6 p-4 bg-[rgba(255,84,115,0.08)] border border-[rgba(255,84,115,0.2)] rounded-xl flex items-center gap-3">
          ...
          <Link href="/approvals" className="btn btn-p btn-sm">Review Now</Link>
        </div>
      )}
      ...
      {brands.length === 0 && (
        <div className="card p-6 mb-6 border-dashed border-2 border-[rgba(255,84,115,0.3)] text-center">
          ...
          <div className="flex gap-3 justify-center">
            ...
            <Link href="/compose" className="btn btn-o">Create Post</Link>
          </div>
        </div>
      )}
      ...
      {posts.length > 0 && (
        <div className="card mt-6">
          <div className="flex items-center justify-between p-4 border-b border-[rgba(90,64,66,0.2)]">
            ...
            <Link href="/sent" className="btn btn-o btn-sm">View All</Link>
          </div>
          <div className="p-4 space-y-3">
            {[...posts]
